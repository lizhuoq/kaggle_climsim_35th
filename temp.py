...
use_cols = [f'{c}_{i}' for c in ["state_t", 'state_q0001', 'state_q0002', 'state_q0003', 'state_u', 'state_v', 'pbuf_ozone', 'pbuf_CH4', 'pbuf_N2O'] for i in range(60)] + [
    'pbuf_SOLIN', "cam_in_ASDIF", "pbuf_COSZRS", "cam_in_ASDIR", "cam_in_LWUP", "cam_in_ALDIF", "cam_in_ALDIR"
]
...
def feature_engineering_version1(data: pl.DataFrame) -> pl.DataFrame:
    ...
    # 温度、湿度和气体梯度 自上向下
    for c in ["state_t", 'state_q0001', 'state_q0002', 'state_q0003', 'pbuf_ozone', 'pbuf_CH4', 'pbuf_N2O']:
        df = data[[f"{c}_{i}" for i in range(60)]].to_pandas().diff(axis=1).fillna(method="bfill", axis=1)
        df = pl.DataFrame(df.to_numpy(), schema=[f"top_bottom_{c}_grad_{i}" for i in range(60)])
        df = df[[f"top_bottom_{c}_grad_{i}" for i in range(60)]]
        res.append(df)
    ...
    # 风梯度
    df = df[[f'wind_speed_{i}' for i in range(60)]].to_pandas().diff(axis=1).fillna(method='bfill', axis=1)
    df = pl.DataFrame(df.to_numpy(), schema=[f'top_bottom_wind_speed_grad_{i}' for i in range(60)])
    df = df[[f'top_bottom_wind_speed_grad_{i}' for i in range(60)]]
    res.append(df)

    # 气体梯度 自下而上
    for c in ['pbuf_ozone', 'pbuf_CH4', 'pbuf_N2O']:
        df = data[[f"{c}_{i}" for i in range(59, -1, -1)]].to_pandas().diff(axis=1).fillna(method="bfill", axis=1)
        df = pl.DataFrame(df.to_numpy(), schema=[f"{c}_grad_{i}" for i in range(59, -1, -1)])
        df = df[[f"{c}_grad_{i}" for i in range(60)]]
        res.append(df)
    ...
    # 辐射*反照率
    df = pl.DataFrame(data[["pbuf_SOLIN"]].to_numpy() * data[["pbuf_COSZRS"]].to_numpy(), schema=["pbuf_SOLIN * pbuf_COSZRS"])
    res.append(df)

    df = pl.DataFrame(data[["pbuf_SOLIN"]].to_numpy() * data[["cam_in_ASDIF"]].to_numpy() * data[["pbuf_COSZRS"]].to_numpy(), schema=["pbuf_SOLIN * cam_in_ASDIF * pbuf_COSZRS"])
    res.append(df)

    df = pl.DataFrame(data[["pbuf_SOLIN"]].to_numpy() * data[["cam_in_ASDIR"]].to_numpy() * data[["pbuf_COSZRS"]].to_numpy(), schema=["pbuf_SOLIN * cam_in_ASDIR * pbuf_COSZRS"])
    res.append(df)

    df = pl.DataFrame(data[["cam_in_LWUP"]].to_numpy() * data[["cam_in_ALDIF"]].to_numpy(), schema=["cam_in_LWUP * cam_in_ALDIF"])
    res.append(df)

    df = pl.DataFrame(data[["cam_in_LWUP"]].to_numpy() * data[["cam_in_ALDIR"]].to_numpy(), schema=["cam_in_LWUP * cam_in_ALDIR"])
    res.append(df)
