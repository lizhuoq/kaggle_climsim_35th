...
class Exp_ED(Exp_Basic):
    def __init__(self, args):
        super(Exp_ED, self).__init__(args)
    ...
    def vali(self, vali_data, vali_loader, criterion, r2=True):
        ...
        pbar = tqdm(total=len(vali_loader))
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y) in enumerate(vali_loader):
                ...
                # outputs = torch.zeros(batch_y.shape[0], 1, self.args.out_channel).float().to(self.device)
                # for i in range(60):
                #     step_out = self.model(batch_x, outputs)
                #     outputs = torch.cat([outputs, step_out[:, -1:, :]], dim=1)
                # outputs = outputs[:, 1:, :]

                teacher_batch_y_0 = batch_y[:, :6 * 60].reshape(-1, 6, 60).transpose(1, 2)
                teacher_batch_y_1 = batch_y[:, 6 * 60:].unsqueeze(1).repeat(1, 60, 1)
                teacher_batch_y = torch.cat([teacher_batch_y_0, teacher_batch_y_1], dim=2)
                outputs = self.model(batch_x, 
                                     torch.cat([torch.zeros(batch_y.shape[0], 1, self.args.out_channel).float().to(batch_y.device), teacher_batch_y[:, :-1, :]], dim=1), 
                                     tgt_is_causal=True)
    ...
    def submit(self, setting, test=0):
        ...
        pbar = tqdm(total=math.ceil(len(df_test) // self.args.batch_size))
        preds = []
        self.model.eval()
        with torch.no_grad():
            for chunk in df_test.iter_slices(self.args.batch_size):
                ...
                outputs = torch.zeros(batch_x.shape[0], 1, self.args.out_channel).float().to(self.device)
                for i in range(60):
                    step_out = self.model(batch_x, outputs)
                    outputs = torch.cat([outputs, step_out[:, -1:, :]], dim=1)
                outputs = outputs[:, 1:, :]
