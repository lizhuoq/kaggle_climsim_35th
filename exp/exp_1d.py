...
class Exp_1D(Exp_Basic):
    ...
    def _select_optimizer(self):
        if self.args.optimizer == "adam":
            ...
            model_optim = optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.args.learning_rate, weight_decay=self.args.weight_decay)
            return model_optim
        elif self.args.optimizer == "sgd":
            print("Optimizer: SGD")
            model_optim = optim.SGD(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.args.learning_rate, momentum=0.9, weight_decay=self.args.weight_decay)
            return model_optim
    ...
    def finetune(self, setting):
        ...
        try:
            ...
            assert self.args.finetune_vali_loss is not None
