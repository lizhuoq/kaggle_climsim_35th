...
from utils.tools import EarlyStopping, visual, adjust_lstm_dropout, adjust_transformer_dropout
...
class Exp_FECV(Exp_Basic):
    def __init__(self, args):
        super(Exp_FECV, self).__init__(args)

    def _build_model(self):
        model: nn.Module = self.model_dict[self.args.model].Model(self.args).float()
        if self.args.custom_parameters:
            for name, param in model.named_parameters():
                if 'attention' not in name and 'projection' not in name:
    ...
    def _select_optimizer(self):
        if self.args.optimizer == "adam":
            ...
            print("Optimizer: SGD")
            model_optim = optim.SGD(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.args.learning_rate, momentum=0.9)
            return model_optim
    ...
    def vali(self, vali_data, vali_loader, criterion, r2=True):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y) in enumerate(vali_loader):
                ...
                batch_x_0 = batch_x[:, :15 * 60].reshape(-1, 15, 60).transpose(1, 2) # B, L, C
                batch_x_1 = batch_x[:, 15 * 60:].unsqueeze(1).repeat(1, 60, 1)
    ...
    def train(self, setting):
        self.args.add_val_data = True
        train_data_1, _ = self._get_data(flag='val')
        self.args.add_val_data = False
        train_data_2, _ = self._get_data(flag='train')

        vali_data, _ = self._get_data(flag='val')
        train_data = torch.utils.data.ConcatDataset([train_data_1, train_data_2, vali_data])
        print(len(train_data))
        kf = KFold(n_splits=5, shuffle=True, random_state=2024)
        for fold, (train_set, val_set) in enumerate(kf.split(train_data)):
            if fold + 1 == self.args.cv_fold:
                break
        print(f"Fold: {fold + 1}")
        train_set = torch.utils.data.Subset(train_data, train_set)
        val_set = torch.utils.data.Subset(train_data, val_set)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y) in enumerate(train_loader):
                ...
                loss = criterion(outputs[:, cal_grad_targets], batch_y[:, cal_grad_targets]).mean(dim=0)
                loss = (loss * torch.tensor(vali_weight).float().to(self.device)).sum()
            ...
            if self.args.model == "LSTM_attention":
                adjust_lstm_dropout(self.model, epoch + 1, self.args.late_dropout_epoch, self.args.late_dropout_p)
            elif self.args.model == "CNN_Transformer":
                adjust_transformer_dropout(self.model, epoch + 1, self.args.late_dropout_epoch, self.args.late_dropout_p)
